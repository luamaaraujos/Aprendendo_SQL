...
--Inicialmente contamos quantos funcionários têm em cada departamento, uma vez que a cláusula é OR
SELECT COUNT(*), departamento
FROM FUNCIONARIOS
group by departamento
order by 1;

--O resultado revelou que temos as seguintes quantidades de funcionários por departamento: 
--> 21 | Filmes   53 | Roupas
...
Adicionei uma pequena camada de "dificuldade" ao exercício, ordenando por departamento*/
...
--Para saber quantas pessoas trabalham no setor lar e no setor Filmes, inicialmente foi solicitada tal informação.
--Isso porque já havia notado previamente que a quantidade de mulheres era maior que a quantidade de funcionários desses dois setores
select count(*), departamento
from funcionarios
group by departamento;

--Nesse caso, trabalharemos com OR e AND ao mesmo tempo para que a cláusula possa ser completa
...
--Como eu já sei que tem muito mais homens no banco, do que funcionários no setor jardim, eu inicio com ele. 
SELECT idFuncionario, nome, email, sexo, departamento, admissao, salario, cargo, idRegiao FROM FUNCIONARIOS
